...
def save_training_state(agent, epsilon, filename="model.save/dqn_training_state.pth"):
...
def load_training_state(agent, filename="model.save/dqn_training_state.pth"):
    if os.path.exists(filename):
        checkpoint = torch.load(filename)
        agent.model.load_state_dict(checkpoint['model_state_dict'])
        agent.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        epsilon = checkpoint['epsilon']
        agent.model.to(device)  # Move model to GPU if available
        print(f"Training state loaded from {filename}")
        return epsilon
    else:
        print(f"No saved training state found at {filename}")
        return 1.0  # Default epsilon value if no checkpoint exists
...
if __name__ == "__main__":
    ...
    parser.add_argument("--random", action="store_true", help="Flag to play against a random White AI")
    args = parser.parse_args()

    agent = DQNAgent(state_size=64, action_size=64)
    agent.model.to(device)  # Transfer model to GPU if available

    # Load model if flag is set
if args.load:
    epsilon = load_training_state(agent)
else:
    epsilon = 1.0  # Start with full exploration

agent.epsilon = epsilon  # Set the agent's epsilon to the loaded value or default
...
with open("model.save/episode_results.txt", "w") as result_file:
    for episode in range(episodes):
        ...
        while not done:
            ...
            if turn == RED:
                # Red team (AI) selects action using DQN agent
                ...
                # White team (random AI) selects action randomly if --random flag is set
                if args.random:
                    action_idx = random.choice(range(len(valid_actions)))
                else:
                    action_idx = agent.get_action(state, len(valid_actions))
        ...
        episode_result = f"Episode {episode + 1}/{episodes} completed. Epsilon: {agent.epsilon}, Total Reward (RED): {total_reward_red}, Total Reward (WHITE): {total_reward_white}\n"
        print(episode_result)
        result_file.write(episode_result)
